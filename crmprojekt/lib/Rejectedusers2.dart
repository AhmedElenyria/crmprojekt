...
class Rejectedusers2 extends StatefulWidget {
  @override
  _DataTableExampleState createState() => _DataTableExampleState();
}

class _DataTableExampleState extends State<Rejectedusers2> {
  ...
  void initState() {
    ...
    fetchData().then((data) {
      setState(() {
 _data = data.where((rowData) => rowData['status'] != 'aktiv' && rowData['status'] != 'Aktivierung ausstehend' && rowData['status'] != 'Loeschung ausstehend' && rowData['admin']=='admin2').toList();
          ...
          rows: _data.asMap().entries.map((entry) {
            ...
            return DataRow(
              cells: [
                ...
                //PasswordDataCell(password: rowData['password']),
...
void function1(BuildContext context,int userId, int rowIndex, String name, String username, String email, String password) async {
  try {
   ...
   Navigator.pushReplacement(
    context,MaterialPageRoute(builder: (BuildContext context) => Rejectedusers2()),
  );
}
