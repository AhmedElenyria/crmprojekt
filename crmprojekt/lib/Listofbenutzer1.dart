...
class Listofbenutzer1 extends StatefulWidget {
  @override
  _DataTableExampleState createState() => _DataTableExampleState();
}

class _DataTableExampleState extends State<Listofbenutzer1> {
  ...
  void initState() {
    ...
    fetchData().then((data) {
      setState(() {
      // List<Map<String, dynamic>> filteredData = data.where((rowData) => rowData['status'] != 'aktiv').toList();
  ...
  onPressed: () {
    Navigator.pushReplacementNamed(context, '/DashboardPage1');
  },
),*/
      ),
...
DataCell(
               ...
               DataCell(
                ElevatedButton(
                                   onPressed: rowButtonEnabledList[index] && ((rowData['status'] == "aktiv") || 
...
try{
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // Die Benutzeroberfläche basierend auf dem tatsächlichen Ergebnis aktualisieren
      setState(() {
        _data[rowIndex]['status'] = 'aktiv';
      ...
      print("Fehler beim Aktualisieren des Benutzerstatus");
      // Den Zustand des Buttons rückgängig machen, wenn das Update fehlschlägt
      setState(() {
...
try {
    ...
    if (response.statusCode == 200) {
     ...
     setState(() {
        rowButtonEnabledList[rowIndex] = true;
        _data[rowIndex]['status'] ='Aktivierung ausstehend';
      });
    }
  } catch (e) {
    print("Error: $e");
...
Navigator.pushReplacementNamed(
      context,
      MaterialPageRoute(builder: (BuildContext context) => Listofbenutzer1()), // Replace YourPage with the actual name of your page class
    );
*/
...
void function1(int userId, int rowIndex,String name,String username,String email,String password) async {
  ...
  try {
    final apiUrl = "http://localhost:8080/api/$userId/updateStatus2";
    final response = await http.post(
      Uri.parse(apiUrl),
      body: {
...
Navigator.pushReplacement(
      ...
      MaterialPageRoute(builder: (BuildContext context) => Listofbenutzer1()), // Replace YourPage with the actual name of your page class
         ...
         //Navigator.pushReplacementNamed(context, '/ListofBenutzer');
}
          ...
          TextButton(
            onPressed: () {
              ...
              //Navigator.push(context, MaterialPageRoute(builder: (context) => Useranlegen1()));
               Navigator.pushReplacementNamed(context, '/Listofbenutzer1');
            },
            child: Text('OK'),
          ),
        ],
